...
const createHeatpointMarker = (lat, lon, latEnd, lonEnd, color) => {
  var latlngs = [
    [lat, lon],
    [latEnd, lonEnd],
  ];
...
function createIconMarker(
  ...
  return L.marker([latEnd, lonEnd], { icon });
}
...
function createMarkersLayer(options, { color, points }) {
  ...
  // create markers
  each(points, ({ lat, lon, latEnd, lonEnd, row }) => {
    const rowCopy = clone(row);
    console.log("options: ", options);
    console.log("rowCopy", rowCopy);
    rowCopy[options.latColName] = lat;
    rowCopy[options.lonColName] = lon;
    rowCopy[options.latEndColName] = latEnd;
    rowCopy[options.lonEndColName] = lonEnd;

    let marker;
    if (classify) {
      console.log("classify");
      marker = createHeatpointMarker(lat, lon, latEnd, lonEnd, color);
    } else {
      if (customizeMarkers) {
        marker = createIconMarker(lat, lon, latEnd, lonEnd, options);
      } else {
